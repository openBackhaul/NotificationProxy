...
exports.putStringProfileStringValue = async function (body, url, uuid) {
  try {
    ...
    if (stringNameForUuid === "kafkaNotificationProvisioningSwitch") {
      let stringValue = body["string-profile-1-0:string-value"];
      if (stringValue === "on" || stringValue === "off") {
        await notificationManagement.updateKafkaNotificationProvisioningSwitch(stringValue);
      } else {
        throw new createHttpError(400, "invalid input in request-body");
      }
    }
